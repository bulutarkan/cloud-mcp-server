...
class MemoryToolTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix="cloud-mcp-memory-test-")
        self.root = Path(self.temp.name) / "memory"
        self.old_env = os.environ.get("CLOUD_MCP_MEMORY_DIR")
        self.old_embed = os.environ.get("CLOUD_MCP_MEMORY_EMBEDDING")
        self.old_model_cache = os.environ.get("CLOUD_MCP_MEMORY_MODEL_CACHE")
        self.old_idle = os.environ.get("CLOUD_MCP_MEMORY_MODEL_IDLE_SECONDS")
        os.environ["CLOUD_MCP_MEMORY_DIR"] = str(self.root)
        os.environ["CLOUD_MCP_MEMORY_EMBEDDING"] = "feature_hash"
        os.environ["CLOUD_MCP_MEMORY_MODEL_CACHE"] = str(Path(self.temp.name) / "model-cache")

    def tearDown(self):
        if self.old_env is None:
            os.environ.pop("CLOUD_MCP_MEMORY_DIR", None)
        else:
            os.environ["CLOUD_MCP_MEMORY_DIR"] = self.old_env
        if self.old_embed is None:
            os.environ.pop("CLOUD_MCP_MEMORY_EMBEDDING", None)
        else:
            os.environ["CLOUD_MCP_MEMORY_EMBEDDING"] = self.old_embed
        if self.old_model_cache is None:
            os.environ.pop("CLOUD_MCP_MEMORY_MODEL_CACHE", None)
        else:
            os.environ["CLOUD_MCP_MEMORY_MODEL_CACHE"] = self.old_model_cache
        if self.old_idle is None:
            os.environ.pop("CLOUD_MCP_MEMORY_MODEL_IDLE_SECONDS", None)
        else:
            os.environ["CLOUD_MCP_MEMORY_MODEL_IDLE_SECONDS"] = self.old_idle
    ...
    def test_memory_add_creates_daily_markdown_and_sqlite_index(self):
        result = self.add_at(
            ...
            tags=["browser", "Cloud MCP"], importance="high", source="conversation",
        )
    ...
    def test_search_get_and_date_range(self):
        ...
        b = self.add_at(datetime(2026, 9, 6, 10, 0, tzinfo=IST), "Cloud MCP server automation should batch Sahibinden filters.", tags=["cloud-mcp", "server"], importance="high")
    ...
    def test_multilingual_search_reindexes_existing_vectors(self):
        ...
        os.environ["CLOUD_MCP_MEMORY_EMBEDDING"] = "auto"
    ...
    def test_fastembed_idle_seconds_is_bounded_and_configurable(self):
        os.environ["CLOUD_MCP_MEMORY_MODEL_IDLE_SECONDS"] = "0"
        self.assertEqual(0.0, memory._fastembed_idle_seconds())
        os.environ["CLOUD_MCP_MEMORY_MODEL_IDLE_SECONDS"] = "12.5"
        self.assertEqual(12.5, memory._fastembed_idle_seconds())
        os.environ["CLOUD_MCP_MEMORY_MODEL_IDLE_SECONDS"] = "99999"
