...
class SkillToolTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix="cloud-mcp-skills-test-")
        self.root = Path(self.temp.name) / "skills"
        self.external = Path(self.temp.name) / "external"
        self.old = {key: os.environ.get(key) for key in [
            "CLOUD_MCP_SKILLS_DIR", "CLOUD_MCP_EMBEDDING",
            "CLOUD_MCP_EMBEDDING_MODEL_CACHE", "CLOUD_MCP_EMBEDDING_IDLE_SECONDS",
        ]}
        os.environ["CLOUD_MCP_SKILLS_DIR"] = str(self.root)
        os.environ["CLOUD_MCP_EMBEDDING"] = "feature_hash"
        os.environ["CLOUD_MCP_EMBEDDING_MODEL_CACHE"] = str(Path(self.temp.name) / "cache")
        os.environ["CLOUD_MCP_EMBEDDING_IDLE_SECONDS"] = "0"
