...
def embedding_mode() -> str:
    return os.getenv(
        "CLOUD_MCP_EMBEDDING",
        os.getenv("CLOUD_MCP_MEMORY_EMBEDDING", "auto"),
    ).strip().lower()
...
def model_cache() -> Path:
    raw = os.getenv(
        ...
        os.getenv("CLOUD_MCP_MEMORY_MODEL_CACHE", ""),
    ).strip()
...
def idle_seconds() -> float:
    raw = os.getenv(
        "CLOUD_MCP_EMBEDDING_IDLE_SECONDS",
        os.getenv("CLOUD_MCP_MEMORY_MODEL_IDLE_SECONDS", "60"),
    ).strip()
    try:
        value = float(raw)
...
def _start_worker_locked() -> Optional[subprocess.Popen[str]]:
    ...
    if not worker.exists():
        return None
    env = os.environ.copy()
    env["CLOUD_MCP_EMBEDDING_MODEL_CACHE"] = cache_key
    env["CLOUD_MCP_EMBEDDING_IDLE_SECONDS"] = str(idle_seconds())
    env["CLOUD_MCP_EMBEDDING_MODEL"] = MULTILINGUAL_MODEL
    env["CLOUD_MCP_EMBEDDING_MODEL_DIMS"] = str(MULTILINGUAL_DIMS)
