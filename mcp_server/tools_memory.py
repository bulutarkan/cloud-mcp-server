...
def _memory_root() -> Path:
    raw = os.getenv("CLOUD_MCP_MEMORY_DIR", "").strip()
    root = Path(raw).expanduser() if raw else (Path.home() / ".cloud-mcp" / "memory")
    root = root.resolve()
...
def _fastembed_cache(root: Path) -> Path:
    raw = os.getenv("CLOUD_MCP_MEMORY_MODEL_CACHE", "").strip()
    cache = Path(raw).expanduser() if raw else (Path.home() / ".cloud-mcp" / "cache" / "fastembed")
    cache = cache.resolve()
...
def _fastembed_idle_seconds() -> float:
    raw = os.getenv("CLOUD_MCP_MEMORY_MODEL_IDLE_SECONDS", "60").strip()
    try:
        value = float(raw)
...
def _start_fastembed_worker_locked(root: Path) -> Optional[subprocess.Popen[str]]:
    ...
    if not worker.exists():
        return None
    env = os.environ.copy()
    env["CLOUD_MCP_MEMORY_MODEL_CACHE"] = cache_key
    env["CLOUD_MCP_MEMORY_MODEL_IDLE_SECONDS"] = str(_fastembed_idle_seconds())
    env["CLOUD_MCP_MEMORY_MODEL"] = MULTILINGUAL_MODEL
    env["CLOUD_MCP_MEMORY_MODEL_DIMS"] = str(MULTILINGUAL_DIMS)
...
def _apple_helper(root: Path) -> Optional[Path]:
    mode = os.getenv("CLOUD_MCP_MEMORY_EMBEDDING", "auto").strip().lower()
...
def memory_search(
    ...
    with closing(_connect(root)) as conn:
        ...
        backend_info: Dict[str, Any] = {
            ...
            "fallback": [FEATURE_VECTOR_BACKEND],
        }
