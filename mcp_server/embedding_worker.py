...
MODEL = os.getenv(
    "CLOUD_MCP_EMBEDDING_MODEL",
    os.getenv("CLOUD_MCP_MEMORY_MODEL", "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2"),
)
DIMS = int(os.getenv("CLOUD_MCP_EMBEDDING_MODEL_DIMS", os.getenv("CLOUD_MCP_MEMORY_MODEL_DIMS", "384")))
CACHE = Path(
    os.getenv(
        "CLOUD_MCP_EMBEDDING_MODEL_CACHE",
        os.getenv("CLOUD_MCP_MEMORY_MODEL_CACHE", str(Path.home() / ".cloud-mcp" / "cache" / "fastembed")),
    )
).expanduser().resolve()
try:
    IDLE_SECONDS = max(
        0.0,
        min(
            float(os.getenv("CLOUD_MCP_EMBEDDING_IDLE_SECONDS", os.getenv("CLOUD_MCP_MEMORY_MODEL_IDLE_SECONDS", "60"))),
            3600.0,
        ),
    )
except ValueError:
